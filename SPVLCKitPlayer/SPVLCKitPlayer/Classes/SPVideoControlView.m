...
@interface SPVideoControlView ()
@property (nonatomic, strong) UIPanGestureRecognizer *pan;
@end

@implementation SPVideoControlView
...
- (void)layoutSubviews {
    ...
    self.alertLbl.center        = CGPointMake(CGRectGetWidth(self.bounds) / 2, CGRectGetHeight(self.bounds) / 2);
...
- (void)panAction:(UIPanGestureRecognizer *)pan {
    ...
    switch (pan.state) {
        ...
        case UIGestureRecognizerStateBegan: {
            ...
            self.alertLbl.alpha = MRVideoControlAlertAlpha;
        ...
        case UIGestureRecognizerStateChanged: {
            ...
            if (ABS(speedDir.x) > ABS(speedDir.y)) {
                if ([pan translationInView:self].x > 0) {
                    ...
                    [self.alertLbl configureWithTime:[self.timeLabel.text substringToIndex:5] isLeft:NO];
                }else {
                    if ([_delegate respondsToSelector:@selector(controlViewFingerMoveRight)]) {
                        [self.delegate controlViewFingerMoveLeft];
                    }
                    [self.alertLbl configureWithTime:[self.timeLabel.text substringToIndex:5] isLeft:YES];
                }
            }else {
                ...
                if (localPoint.x > self.bounds.size.width / 2) {
                    ...
                    [self.alertLbl configureWithVolume:self.volumeSlider.value];
                }else {
                    // 改变显示亮度
                    if ([pan translationInView:self].y > 0) {
                    ...
                    [self.alertLbl configureWithLight];
                }
            }
        }
            break;
...
- (SPVideoHUDView *)indicatorView {
    if (!_indicatorView) {
        _indicatorView = [[SPVideoHUDView alloc] init];
...
- (SPProgressSlider *)progressSlider {
    if (!_progressSlider) {
        _progressSlider = [[SPProgressSlider alloc] init];
...
- (UILabel *)alertLbl {
    if (!_alertLbl) {
        _alertLbl = [UILabel new];
        _alertLbl.bounds = CGRectMake(0, 0, 100, 40);
        _alertLbl.textAlignment = NSTextAlignmentCenter;
        _alertLbl.backgroundColor = [UIColor colorWithWhite:0.000 alpha:MRVideoControlAlertAlpha];
        _alertLbl.textColor = [UIColor whiteColor];
        _alertLbl.layer.cornerRadius = 10;
        _alertLbl.layer.masksToBounds = YES;
        _alertLbl.alpha = 0;
    }
    return _alertLbl;
}
...
@implementation SPProgressSlider
