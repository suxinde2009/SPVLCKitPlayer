...
@interface SPVideoHUDView () {
    CAShapeLayer *_leftLayer;
    CAShapeLayer *_rightLayer;
}
@end

@implementation SPVideoHUDView
- (instancetype)init {
    if (self = [super init]) {
...
@interface SPVideoAlertView ()

@end
@implementation SPVideoAlertView

+ (instancetype)shareInstance {
    static SPVideoAlertView *alertView = nil;
    static dispatch_once_t predicate;
    dispatch_once(&predicate, ^{
        alertView = [[SPVideoAlertView alloc] init];
    });
    return alertView;
}
...
- (instancetype)init {
    if (self = [super init]) {
        _msgLbl = [UILabel new];
        _msgLbl.frame = self.bounds;
        _msgLbl.backgroundColor = [UIColor redColor];
